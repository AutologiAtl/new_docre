...
class INTERASIAExcelWriter:
        ...
        try:
            ...
            try:				
                if len(final_values) > 4:
                    ...
                    print(f"bl_issue_from_excel---{bl_issue_from_excel}")
    ...
    def write_sheet_2(self, workbook):
        ...
        workbook.sheets[1].range(f'H{last_row}').formula = sum_formula
        workbook.sheets[1].range(f'H{last_row + 1}').value = "KG"
            

        # Sum values in column F
        sum_formula = f'=SUM(L11:L{last_row - 1})'
        workbook.sheets[1].range(f'L{last_row}').formula = sum_formula
        workbook.sheets[1].range(f'L{last_row +1}').value = "M3"
